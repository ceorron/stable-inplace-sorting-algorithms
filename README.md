...
A well implemented algorithm does exactly n comparisons, and at worst approximately (1/2n(1/4n)) swaps.
...
Because a well implemented algorithm needs to move items around more than 1 at a time this means that the algorithm makes use of a smaller ammount of stack allocated "scratch space" for the efficient movement of multiple items between the buffers (specifically from the left buffer to the middle buffer). This is constant in size, as it set at the larger of ((1 x sizeof(T)) or 2048) bytes. This means the algorithm can move (trun(2048/sizeof(T)) or 1) items in a single iteration.
